...
def initialize(request):
    ...
    return JsonResponse({'uuid': uuid, 'name':player.user.username, 'coord': {'x': room.x, 'y': room.y}, 'title':room.title, 'description':room.description, 'players':players}, safe=True)
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        for p_uuid in currentPlayerUUIDs:
            pusher.trigger(f'p-channel-{p_uuid}', 'broadcast', {
                           'info': f'{player.user.username} has walked {dirs[direction]}.'})
        for p_uuid in nextPlayerUUIDs:
            pusher.trigger(f'p-channel-{p_uuid}', 'broadcast', {
                           'info': f'{player.user.username} has entered from the {reverse_dirs[direction]}.'})
        ...
        players = room.playerNames(player_id)
        return JsonResponse({'name':player.user.username, 'coord': {'x': room.x, 'y': room.y}, 'title':room.title, 'description':room.description, 'players':players, 'error_msg':"You cannot move that way."}, safe=True)
